...
init -1 style namebox:
    ...
    background Image("gui/namebox.png", xalign=0.5, yalign=1.0)
    padding gui.namebox_borders.padding

init -1 style say_label:
    properties gui.text_properties("name", accent=True)
    xalign gui.name_xalign
    yanchor gui.name_yanchor
    ypos gui.name_ypos
    xoffset gui.name_xoffset
...
init -1 python:
    ...
    def get_current_bg():
        ...
        if all_bg_images is None:
            
            all_bg_images = filter(lambda i: i.startswith('bg '), renpy.list_images())
        for img in all_bg_images:
            if renpy.showing(img):
                return img
...
init -501 screen about():
    ...
    use game_menu(_("About"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only] and {a=http://godotengine.org/}Godot Engine{/a} (visual tool).\n\n[renpy.license!t]")
...
init -1 python:
    class GlossaryWord(object):
        def __init__(self, words={}, texts={}):
            self.words = words
            self.texts = texts
        @property
        def word(self):
            return self.words.get(_preferences.language, None) or '□'
        @property
        def text(self):
            return self.texts.get(_preferences.language, None) or '□'
    ...
    glossary_texts = [
        GlossaryWord(
            {EN:'Aecet', JP:'エセット'},
            {
                EN:'The Aecet tribe was a race that resembles humans, but with pure white skin, pure white hair, and crimson eyes.\nTheir culture was very strongly tied to music. They were very advanced, and could use various sounds to create miracles.',
                JP:'エセット族は純白の肌、真っ白な髪、そして真紅の眼を持つ人間と良く似た種族だ。\n音楽を基調とする文化を形成していた。\n彼らは高度な文化をもち、音を使ってさまざまな奇跡を起こしたという。'
            }
        ),
        GlossaryWord(
            {EN:'Forest of Death', JP: '死の森'},
            {
                EN: 'Many vicious beasts live here.\nThey will attack and eat away to the bone any person that dares to step foot in the forest.\nIt is also a hotbed for various kinds of viruses.\nAfter you step foot in these lands, you likely won\'t be able to return alive.',
                JP: '森の中には凶暴な獣が数多く生息しており、\n足を踏み入れた人間を襲い骨まで食らい尽くす。\nまた様々な病原菌の温床でもある。\n足を踏み入れたら最後、生きて出ることはできない。'
            }
        ),
        GlossaryWord(
            {EN:'Song of life', JP:'命の歌'},
            {
                EN:'A song that brings out the power of the red stone.\nIt was created by accident, when the Aecets were trying to concoct the Elixir of life.\nIt has to be played by an ocarina.',
                JP:'赤の活力の力を引き出す音楽。\nエセットを不老不死にさせる道具を作る過程で生成された。\nオカリナが奏でる命の歌によって機能する。'
            }
        ),
        GlossaryWord(
            {EN:'Red Stone', JP:'赤の活力'},
            {
                EN:'A red, round jewel, made by the Aecets. It has the power the revive the dead.',
                JP:'エセット族が作り上げた赤くて丸い宝石。死者を蘇らせる特殊な力がある。'
            }
        )
    ]

init -501 screen glossary():
    tag menu
    ...
    use game_menu(_("Glossary"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                for w in range(len(glossary_texts)):
                    textbutton _(glossary_texts[w].word) action SetScreenVariable("gword", w)

            hbox:

                text _(glossary_texts[gword].text)


init -1 style glossary_button is help_button
init -1 style glossary_button_text is help_button_text
init -1 style glossary_label is help_label
init -1 style glossary_label_text is help_label_text
init -1 style glossary_text is help_text

init -1 style glossary_button_text:
